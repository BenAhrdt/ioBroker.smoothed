...
class statehandlingtypes {
    ...
    async createInternalValues() {
        ...
        for (const channelName in this.adapter.activeChannels) {
            ...
            this.adapter.log.warn(channelName);
            this.adapter.log.warn(JSON.stringify(channel));
            // create State for the name
            await this.adapter.setObjectAsync(this.generateInternalChannelString(channelName), {
                type: 'channel',
                common: {
                    name: channelName,
                    desc: channel.id,
