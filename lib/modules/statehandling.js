	...
	async createInternalValues(){
				...
				if(resultObject.common.type === "number"){
					...
					if(elementName === element.name){
						...
						if(resultObject && resultState){
							...
							// Limit initial value
							if(this.adapter.activeChannels[element.name].limitInNegativeDirection && this.adapter.activeChannels[element.name].currentValue < this.adapter.activeChannels[element.name].negativeLimit){
								this.adapter.activeChannels[element.name].currentValue = this.adapter.activeChannels[element.name].negativeLimit;
								this.adapter.activeChannels[element.name].smoothed = this.adapter.activeChannels[element.name].negativeLimit;
							}
							else if(this.adapter.activeChannels[element.name].limitInPositiveDirection && this.adapter.activeChannels[element.name].currentValue > this.adapter.activeChannels[element.name].positiveLimit){
								this.adapter.activeChannels[element.name].currentValue = this.adapter.activeChannels[element.name].positiveLimit;
								this.adapter.activeChannels[element.name].smoothed = this.adapter.activeChannels[element.name].positiveLimit;
							}
						...
						this.adapter.log.warn(`The given name ${element.name} with th id ${element.id} is not a valid name. Please change it.`);
					}
				}
				else{
					this.adapter.log.warn(`The selected id ${element.id} in the config with the name ${element.name} is not a number`);
				}
			}
			catch{
		...
		for(const channelName in this.adapter.activeChannels){
			...
			if((!channel.lastArray.smoothtime || channel.smoothtime !== channel.lastArray.smoothtime) ||
				...
				channel.lastArray.value.push({val:channel.currentValue,ts:Date.now() - timedifference},{val:channel.currentValue,ts:Date.now()});
